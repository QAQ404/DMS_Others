...
-- Records of building
-- ----------------------------
INSERT INTO `building` VALUES ('1', '1栋', '2', '2', '16', '1');
...
INSERT INTO `building` VALUES ('14', '临时宿舍楼', '1', '999', '1', '7');
...
CREATE TABLE `change_dormitory_application` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='申请换寝表';

-- ----------------------------
-- Records of change_dormitory_application
-- ----------------------------
INSERT INTO `change_dormitory_application` VALUES ('1', '8', '2', '-1', null, null, '0', '2024-02-26 16:03:00', '2024-02-26 16:03:03');
...
-- ----------------------------
INSERT INTO `dormitory` VALUES ('1', '14', '暂未分配寝室', '1', '7', '99999', '2024-02-14 10:07:11', '1', null);
INSERT INTO `dormitory` VALUES ('2', '1', '101', '1', '1', '4', '2024-02-19 21:13:05', '1', 'https://hzh-biyesheji.oss-cn-beijing.aliyuncs.com/4197b720-6085-4389-87d7-cf284430aa7f.jpg');
...
-- Records of student
-- ----------------------------
INSERT INTO `student` VALUES ('8', '1', '202001010101', '男', '1', '1', '1', '2020');
INSERT INTO `student` VALUES ('11', '1', '202001010102', '男', '1', '1', '1', '2020');
INSERT INTO `student` VALUES ('15', '1', '202001010103', '男', '1', '1', '1', '2020');
...
INSERT INTO `student` VALUES ('18', '2', '202001010202', '男', '1', '5', '1', '2020');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_change_dor_application` AS select `change_dormitory_application`.`id` AS `id`,`change_dormitory_application`.`state` AS `state`,`change_dormitory_application`.`create_time` AS `create_time`,`change_dormitory_application`.`update_time` AS `update_time`,`change_dormitory_application`.`stu_id` AS `stu_id`,`user`.`name` AS `stu_name`,`student`.`study_id` AS `stu_study_id`,`dor1`.`id` AS `old_id`,`dor1`.`name` AS `old_name`,`dor1`.`unit_number` AS `old_unit_number`,`dor1`.`floor_number` AS `old_floor_number`,`building1`.`name` AS `old_building_name`,`dor2`.`id` AS `new_id`,`dor2`.`name` AS `new_name`,`dor2`.`unit_number` AS `new_unit_number`,`dor2`.`floor_number` AS `new_floor_number`,`building2`.`name` AS `new_building_name`,`change_dormitory_application`.`new_stu_id` AS `new_stu_id`,`change_dormitory_application`.`new_stu_name` AS `new_stu_name`,`change_dormitory_application`.`new_stu_study_id` AS `new_stu_study_id` from ((((((`change_dormitory_application` join `student`) join `dormitory` `dor1`) join `dormitory` `dor2`) join `user`) join `building` `building1`) join `building` `building2`) where ((`student`.`user_id` = `change_dormitory_application`.`stu_id`) and (`student`.`dormitory_id` = `dor1`.`id`) and (`change_dormitory_application`.`new_dormitory_id` = `dor2`.`id`) and (`user`.`id` = `student`.`user_id`) and (`dor1`.`building_id` = `building1`.`id`) and (`dor2`.`building_id` = `building2`.`id`)) ;
